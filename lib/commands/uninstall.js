...
class UninstallCommand extends Command {
        ...
        return (prompt || Promise.resolve({sure: true})).then((answer) => {
            ...
            return this.ui.listr([{
                ...
                task: () => {
                    ...
                    if (this.useSudo(contentDir)) {
                        return this.sudoRemovePath(contentDir);
                    }
    ...
    hasSudo() {
        // TODO: detect sudo or posix?
        return (os.platform() === 'linux');
    }

    useSudo(path) {
        try {
            return this.hasSudo() && (process.getuid() !== fs.lstatSync(path));
        } catch (e) {
            // TODO: what?
        }
        return false;
    ...
    sudoRemovePath(path) {
        let options = {};
        if (argv.sudo) {
            options.sudoArgs = `-u ${argv.sudo}`;
        }
        return this.ui.sudo(`rm -rf ${path}`, options);
    }
}
...
UninstallCommand.options = {
    ...
    sudo: {
        description: 'Sudo as specific user',
        type: 'string'
    }
};
